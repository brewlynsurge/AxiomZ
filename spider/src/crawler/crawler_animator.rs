...
impl CrawlerAnimator {
    ...
    pub async fn create_instance(
        &mut self,
        page_url: &str,
    ) -> Result<AnimatorInstance, Box<dyn std::error::Error + Send + Sync>> {
        let tx = self.processing_tx.clone();
        tx.send(DrawProcessingCommands::SendUrl(page_url.to_string()))
            .await?;

        Ok(AnimatorInstance { processing_tx: tx })
    }
}
...
impl DrawProcessingMode {
    pub fn to_string(&self) -> String {
        match self {
            DrawProcessingMode::Processing => String::from("Processing"),
            DrawProcessingMode::Saving => String::from("Saving"),
        }
    }

    pub fn apply_style(&self) -> crossterm::style::StyledContent<String> {
        match self {
            DrawProcessingMode::Processing => String::from("Processing").blue(),
            DrawProcessingMode::Saving => String::from("Saving").green(),
        }
    }
}
...
impl AnimatorInstance {
    ...
    pub async fn to_processing(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        self.processing_tx
            .send(DrawProcessingCommands::StartProcessing)
            .await?;
        Ok(())
    }
    ...
    pub async fn to_saving(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        self.processing_tx
            .send(DrawProcessingCommands::ToSaving)
            .await?;
        Ok(())
    }
    ...
    pub async fn to_success(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        self.processing_tx
            .send(DrawProcessingCommands::Done(DrawProcessingResult::Success))
            .await?;
        Ok(())
    }
    ...
    pub async fn to_failure(
        ...
        self.processing_tx
            .send(DrawProcessingCommands::Done(DrawProcessingResult::Failure(
                error_msg,
            )))
            .await?;
        Ok(())
    ...
    async fn draw_processing_animation(
        mut entry_parameters: DrawProcessingParameters,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        loop {
...

            {
                ...
                let first_layer_msg = format!(
                    "{} 🔗 {}",
                    "Crawling".bold().cyan(),
                    Self::prettify_url(&page_url, Self::PRETTIFY_MAX_LEN)
                );
            ...
            match entry_parameters.rx.recv().await {
                ...
                _ => {
                    panic!("Expected a to_processing command")
                }
            }
    ...
    async fn play_success(
        stdout: &mut tokio::sync::MutexGuard<'_, std::io::Stdout>,
        url: &str,
        animation_row: u16,
        first_time: bool,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if !first_time {
            crossterm::queue!(
                stdout,
                cursor::MoveTo(0, animation_row - 2),
                terminal::Clear(terminal::ClearType::CurrentLine)
            )?;
        }
        ...
        writeln!(
            ...
            "{} 🔗 {}",
            "Success".green().bold(),
            Self::prettify_url(url, Self::PRETTIFY_MAX_LEN)
        )?;
        stdout.flush()?;
        Ok(())
    }

    async fn play_failure(
        stdout: &mut tokio::sync::MutexGuard<'_, std::io::Stdout>,
        url: &str,
        animation_row: u16,
        processing_mode: &DrawProcessingMode,
        error_msg: Option<String>,
        first_time: bool,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        crossterm::queue!(
