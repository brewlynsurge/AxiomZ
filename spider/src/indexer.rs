...
impl Indexer {
    pub fn new(title: &str, url: &str, description: &str) -> Self {
        let description = {
            if description.is_empty() {Some(description.to_string())}
            else {None}
        };
    ...
    pub async fn parse(&self, page_texts: HashSet<String>, surfx_database: &SurfXDatabase) -> Result<(), std::io::Error> {
        ...
        let stop_words = utils::STOP_WORDS.iter()
            .map(|s| s.to_string()).collect::<Vec<String>>();
        let total_webpages = surfx_database.count_total_webpages().await?;

        let regex_word = Regex::new(r"\p{L}[\p{L}\p{N}_]*")
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        let regex_english = Regex::new(r"^[a-zA-Z]+$")
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        ...
        for text in page_texts {
            for word in text.split_whitespace() {
                ...
                let frequency = words_hash.get("").cloned().unwrap_or(0);
        ...
        for (word, freq) in words_hash {
            ...
            let idf = {
                let word_occurence = surfx_database.count_word_occurrences(word_id).await?;
                (total_webpages as f64 / (1 + word_occurence) as f64).log10()
            };
            let tf_idf = tf * idf;
